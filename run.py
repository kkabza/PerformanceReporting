...
def kill_processes_on_port(port, logger):
    ...
    try:
        ...
        if system == 'Windows':
            ...
            # Find processes using the port on macOS
            find_cmd = f"lsof -i :{port} -sTCP:LISTEN -t"
            try:
                pids_output = subprocess.check_output(find_cmd, shell=True, text=True)
                pids = set(int(pid.strip()) for pid in pids_output.splitlines() if pid.strip())
                ...
                # If the command returns non-zero (no processes found), that's fine
                logger.info(f"No processes found using port {port}")
                
        elif system == 'Linux':
            # Find processes using the port on Linux
            find_cmd = f"lsof -i :{port} -sTCP:LISTEN -t"
            try:
                pids_output = subprocess.check_output(find_cmd, shell=True, text=True)
                pids = set(int(pid.strip()) for pid in pids_output.splitlines() if pid.strip())
            ...
            logger.warning(f"Unsupported operating system: {system}. Cannot check for processes using port {port}")
            return
            
        # Give some time for the port to be released
        time.sleep(1)
        
        # Verify the port is now available by trying to bind to it
        import socket
        try:
